...
class Button:
    def __init__(self, SCREEN: pg.Surface, x: float, y: float, size: tuple, bgColor: tuple, text: str, font: pg.font,
        ...
        self.transparentSurf = pg.Surface(self.size)
        self.transparentSurf.set_colorkey((50, 50, 50))
        self.transparentSurf.fill((50, 50, 50))
        self.transparentSurf.set_alpha(100)
        self.transparentSurf = self.transparentSurf.convert_alpha()

        self.opaqueSurf = pg.Surface(self.size)
        self.opaqueSurf.set_colorkey((50, 50, 50))
        self.opaqueSurf.fill((50, 50, 50))
        self.opaqueSurf = self.opaqueSurf.convert()
    ...
    def setButtonSurf(self):
        rect(self.transparentSurf, self.bgColor, pg.Rect(0, 0, *self.size), border_radius=self.btnRound)
        rect(self.opaqueSurf, self.bgColor, pg.Rect(0, 0, *self.size), border_radius=self.btnRound)

        textOffset = self.size[1] / (len(self.text) + 1)

        for idx, text in enumerate(self.text):
            self.transparentSurf.blit(text, (self.size[0] / 2 - self.textSize[idx][0] / 2,
                                             textOffset * (idx + 1) - self.textSize[idx][1] / 2))
            self.opaqueSurf.blit(text, (self.size[0] / 2 - self.textSize[idx][0] / 2,
                                        textOffset * (idx + 1) - self.textSize[idx][1] / 2))
    ...
    def draw(self):
        ...
        if hover:
            self.SCREEN.blit(self.opaqueSurf, (x, y))
        else:
            self.SCREEN.blit(self.transparentSurf, (x, y))
